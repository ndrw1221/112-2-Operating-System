...
sys_open(void)
{
  ...
  struct inode *ip;
  int n;
  int max_symlink_depth = 20; // Prevent infinite loops in symlinks
  int depth = 0;
  ...
  else
  {
    while (depth < max_symlink_depth)
    {
      ...
      // If it's a symbolic link and O_NOFOLLOW is not specified, resolve it
      if (ip->type == T_SYMLINK && !(omode & O_NOFOLLOW))
      {
        ...
        safestrcpy(path, symlink_target, MAXPATH);
        depth++;
      }
      else
      {
        break; // Not a symlink, or O_NOFOLLOW is specified
      }
    }
...
sys_symlink(void)
{
  ...
  // Get the parent directory inode and the name of the symbolic link
  if ((dp = nameiparent(path, name)) == 0)
  {
    end_op();
    return -1;
  ...
  // Link the symbolic link to the directory
  if (dirlink(dp, name, ip->inum) < 0)
  {
    iunlockput(ip);
    goto bad;
