...
bmap(struct inode *ip, uint bn) // ip: inode pointer, bn: block number
{
  ...
  if (bn < NINDIRECT)
  {
    int indirect_index = bn / NADDRS;
    int indirect_offset = bn % NADDRS;
    // Load indirect block, allocating if necessary.
    if ((addr = ip->addrs[NDIRECT + indirect_index]) == 0)
  ...
  if (bn < NDINDIRECT)
  {
    int doubly_indirect_index = bn / NADDRS;
    int double_indirect_offset = bn % NADDRS;
    // Load double indirect block, allocating if necessary.
    if ((addr = ip->addrs[NDIRECT + NINDIRECT]) == 0)
      ip->addrs[NDIRECT + NINDIRECT] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint *)bp->data;
    if ((addr = a[doubly_indirect_index]) == 0)
    {
      a[doubly_indirect_index] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    ...
    bp = bread(ip->dev, addr);
    a = (uint *)bp->data;
    if ((addr = a[double_indirect_offset]) == 0)
    {
      a[double_indirect_offset] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    return addr;
