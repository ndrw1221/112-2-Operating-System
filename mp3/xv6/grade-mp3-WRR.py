...
@test(2, "task1")
def test_uthread():
    r.run_qemu(shell_script(["task1"]), make_args=["SCHEDPOLICY=THREAD_SCHEDULER_WRR"])
...
@test(2, "task2")
def test_uthread():
    r.run_qemu(shell_script(["task2"]), make_args=["SCHEDPOLICY=THREAD_SCHEDULER_WRR"])
    expected = """
dispatch thread#1 at 0: allocated_time=4
...
@test(2, "task3")
def test_uthread():
    r.run_qemu(shell_script(["task3"]), make_args=["SCHEDPOLICY=THREAD_SCHEDULER_WRR"])
...
@test(2, "task4")
def test_uthread():
    r.run_qemu(shell_script(["task4"]), make_args=["SCHEDPOLICY=THREAD_SCHEDULER_WRR"])
